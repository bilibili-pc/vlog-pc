<template>
  <div class="bg row">
    ...
    <v-dialog v-model="flag" max-width="500" v-else>
      <v-card>
        <v-card-title class="headline grey lighten-2">登录成功</v-card-title>
        <v-img height="250" src="https://wshan.oss-cn-beijing.aliyuncs.com/touxiang.jpg"></v-img>
        <v-card-text>现在进入我的个人界面</v-card-text>

        <v-divider></v-divider>

        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="primary" text @click="flag = false">I accept</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
...
.bg {
  width: 100%;
  height: 100vh;
  background: url('https://wshan.oss-cn-beijing.aliyuncs.com/touxiang.jpg');
}
.mask {
